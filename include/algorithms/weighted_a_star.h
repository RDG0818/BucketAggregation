// include/algorithms/weighted_a_star.h
...
public:
  ...
  WeightedAStar(E& env, PQ& priority_queue, utils::SearchStats* stats = nullptr, bool collect_metrics = false, double weight = 1.5) 
    : env_(env), priority_queue_(priority_queue), stats_(stats), collect_metrics_(collect_metrics), weight_(weight) {
      if (collect_metrics_) {
  ...
  void solve() {
    ...
    // Weighted priority: g + w * h
    priority_queue_.push(start_node, (double)0 + weight_ * start_h, start_h);
    ...
    while (!priority_queue_.empty()) {
      ...
      for (uint32_t v : neighbors) {
        ...
        if (new_g < pool.get_g(v)) {
          ...
          // Push with weighted priority
          priority_queue_.push(v, (double)new_g + weight_ * h, h);
