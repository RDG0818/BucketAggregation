// include/algorithms/greedy_best_first_search.h
...
public:
  ...
  GreedyBestFirstSearch(E& env, PQ& priority_queue, utils::SearchStats* stats = nullptr, bool collect_metrics = false, double weight = 1.0) 
    ...
    while (!priority_queue_.empty()) {
      ...
      for (uint32_t v : neighbors) {
        ...
        if (new_g < pool.get_g(v)) {
          ...
          // Push with priority h
          priority_queue_.push(v, h, h);
