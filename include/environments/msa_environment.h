// include/environments/msa_environment.h
...
// Multiple Sequence Alignment (MSA) environment for N sequences.
//
// State is an N-dimensional index vector tracking the current alignment position
// in each sequence. A move advances one or more sequences by 1 (2^N - 1 possible
// moves per state). Edge costs are derived from the PAM250 substitution matrix
// summed over all sequence pairs. Heuristic is a sum of pairwise optimal alignment
// costs, computed once at construction via DP and stored in h_tables_.
template <size_t N>
class MSAEnvironment {

public:

  using T = std::array<uint16_t, N>;
  ...
  void get_successors(uint32_t u_id, std::vector<uint32_t>& neighbors) {
    ...
    for (uint32_t m = 1; m <= num_moves; ++m) {
      ...
      if (valid) neighbors.push_back(get_or_create_id(next_state));
    }
  }
  ...
  uint32_t get_start_node() { return get_or_create_id(start_state_); }
...
private:
  ...
  void compute_pairwise_heuristics() {
    ...
    for (size_t i = 0; i < N; ++i) h_tables_[i].resize(N - 1 - i);
  ...
  uint32_t compute_transition_cost(const T& u_state, const T& v_state) const {
    ...
    uint8_t indices[N];
    for (size_t i = 0; i < N; ++i) {
      indices[i] = (v_state[i] > u_state[i]) ? encoded_sequences_[i][u_state[i]] : 20; // 20 = GAP
    }
    for (size_t i = 0; i < N; ++i) {
      for (size_t j = i + 1; j < N; ++j) {
  ...
  static int aa_to_idx(char c) {
    ...
    return 20; // unknown character -> GAP
  }
