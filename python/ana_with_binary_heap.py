...
from ana_bucket_heap import SearchNode # Re-use SearchNode for consistency
...
def anytime_nonparametric_astar(grid: Grid, start: Node, goal: Node, heuristic):
    """
    Anytime Non-parametric A* (ANA*) search algorithm using a pure Python
    class-based Binary Heap.
    """
    ...
    # Define the potential function for ANA*
    def ana_potential(g, h, C):
        if h == 0: return math.inf
        return (C - g) / h
    ...
    # Use the BinaryHeap for the open set
    open_set = BinaryHeap(ana_potential)
