...
RandomGridEnvironment::RandomGridEnvironment(uint32_t w, uint32_t h, uint32_t max_edge_cost, uint32_t seed) 
  : w_(w), h_(h), goal_id_(w_ * h_ - 1) {
    ...
    std::uniform_int_distribution<> cost_dist(1, max_edge_cost);    

    float obstacle_pct = 0.20f;

    for (uint32_t i = 0; i < total_states; ++i) {
      if (obs_dist(gen) < obstacle_pct) {
        grid_map_[i] = 0; // Obstacle
      } else {
        grid_map_[i] = static_cast<uint8_t>(cost_dist(gen)); // Random Cost
      }
    }

    // Ensure start and goal are passable (force cost 1 to be safe)
    if (grid_map_[0] == 0) grid_map_[0] = 1;
    if (grid_map_[goal_id_] == 0) grid_map_[goal_id_] = 1;
}

void RandomGridEnvironment::get_successors(uint32_t u_id, std::vector<uint32_t>& neighbors) {
  neighbors.clear();
